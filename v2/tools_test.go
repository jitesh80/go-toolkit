...
var jsonTests = []struct {
	...
	{name: "incorrect json type", json: `{"foo": 1}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "two json payloads", json: `{"foo": "1"}{"alpha": "beta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty json body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo": 1"`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "unknown field in json", json: `{"hello": "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allow unknown field in json", json: `{"hello": "1"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field in json", json: `{jack: "1"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
	{name: "file size is too large", json: `{"foo": "bar"}`, errorExpected: true, maxSize: 5, allowUnknown: true},
	{name: "not a valid json", json: `Hello, World!!!`, errorExpected: true, maxSize: 1024, allowUnknown: true},
}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		// declare a variable to read and decode the json into
		var decodedJSON struct {
			Foo string `json:"foo"`
		}

		// create a request with the body set to the json
		...
		log.Println("Decoded JSON:", decodedJSON)
