...
// ReadJSON reads JSON data from the request body and decodes it into the data interface
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	// Allow maximum of 1mb of data
	maxBytes := 1024 * 1024 // one meg

	// If we are passing MaxJSONSize then we allocate that instead of setting a default one
	if t.MaxJSONSize != 0 {
	...
	// Decode JSON data into data interface which we pass into the method
	err := dec.Decode(data)
	if err != nil {
...
// WriteJSON takes a response status code and arbitrary data and sends JSON response
func (t *Tools) WriteJSON(w http.ResponseWriter, status int, data interface{}, headers ...http.Header) error {
	...
	// we write data out
	_, errJson := w.Write(out)
	// log.Println("Json.resp", string(out))
	if errJson != nil {
		return errJson
	}
...
// ErrorJSON takes an error and a status code, and generates and sends a JSON response
